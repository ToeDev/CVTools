...
public class Clear extends Command {
    ...
    public CommandResponse execute(Player sender, Set<String> flags, Map<String, Object> parameters, List<Object> baseParameters) {
        ...
        if(slot && all) { return new CommandResponse("&c-s and -a are exclusive options"); }
        ...
        if(!self) {
            ...
            targetSB.append("&aYour ");
            if(all) { targetSB.append("entire inventory"); }
            else if(slot) { targetSB.append("main hand inventory slot"); }
            else { targetSB.append("inventory"); }
            targetSB.append(" has been cleared.");
            player.sendMessage(targetSB.toString());
        }
