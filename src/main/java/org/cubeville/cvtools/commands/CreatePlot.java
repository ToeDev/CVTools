...
public class CreatePlot extends Command {
    ...
    private final CuboidRegion PLOT_TEMPLATE =
        new CuboidRegion(BukkitAdapter.adapt(CREATIVE_WORLD),
    ...
    public CreatePlot() {
        ...
        setPermission("cvtools.createplot");
    }
    ...
    private void copyPlotToLocation(BlockVector3 loc) throws CommandExecutionException {
        ...
        try (EditSession editSession = WorldEdit.getInstance().getEditSessionFactory()
            ...
            ForwardExtentCopy forwardExtentCopy =
                new ForwardExtentCopy(editSession, PLOT_TEMPLATE, clipboard, PLOT_TEMPLATE.getMinimumPoint());
        ...
        for (int i = 0; i < MAX_PLOTS; i++) {
            ...
            if (gridLocation.getX() + gridLocation.getZ() == 2 * Math.max(gridLocation.abs().getX(), gridLocation.abs().getZ())) {
                ...
                if (gridLocation.abs().getX() == gridLocation.abs().getZ()) {
                    moveDirection = BlockVector2.at(-1 * moveDirection.getZ(),
                                                    moveDirection.getX());
                }
            }
        }
